...
class DynamicRegistration {
  ...
  async register(openidConfiguration, registrationToken, options) {
    ...
    try {
      registrationResponse = await got.post(configuration.registration_endpoint, {
        json: registration,
      ...
      throw new Error(`REMOTE_REGISTRATION_FAILED: ${error.message || error} from endpoint ${configuration.registration_endpoint}` + tokenMessage);
    }
