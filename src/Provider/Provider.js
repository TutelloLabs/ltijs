...
const provDynamicRegistrationDebug = require("debug")(
	"provider:dynamicRegistrationService",
);
...
class Provider {
	...
	#connectCallback = async (token, req, res, next) => {
		return next();
	};

	#deepLinkingCallback = async (token, req, res, next) => {
		return next();
	};
	...
	#inactivePlatformCallback = async (req, res) => {
		return res
			...
			.send({
				status: 401,
				error: "Unauthorized",
				details: { message: "PLATFORM_NOT_ACTIVATED" },
			});
	};
	...
	setup(encryptionkey, database, options) {
		...
		this.#server = new Server(
			options ? options.https : false,
			options ? options.ssl : false,
			this.#ENCRYPTIONKEY,
			options ? options.cors : true,
			options ? options.serverAddon : false,
		);
		 ...
		 */
		this.NamesAndRoles = new NamesAndRolesService(
			this.getPlatform,
			this.#ENCRYPTIONKEY,
			this.Database,
		);
		...
		if (options && options.dynReg) {
			 ...
			 */
			this.DynamicRegistration = new DynamicRegistration(
				options.dynReg,
				routes,
				...
				if (!ltik) {
					...
					if (idtoken) {
						...
						await this.Database.Replace(
							false,
							"idtoken",
							{ iss: valid.iss, clientId, deploymentId, user: valid.sub },
							platformToken,
						);
		...
		this.app.all(this.#loginRoute, async (req, res) => {
			...
			try {
				if (!params.iss || !params.login_hint || !params.target_link_uri)
					return res
						...
						.send({
							status: 400,
							error: "Bad Request",
							details: { message: "MISSING_LOGIN_PARAMETERS" },
						});
				...
				if (platform) {
					...
					const query = await Request.ltiAdvantageLogin(
						params,
						platform,
						state,
					);
		...
		this.app.all(this.#dynRegRoute, async (req, res, next) => {
			...
			return res
				...
				.send({
					status: 403,
					error: "Forbidden",
					details: { message: "Dynamic registration is disabled." },
				});
		});
	...
	onConnect(_connectCallback, options) {
		...
		if (options) {
			if (options.sameSite || options.secure)
				console.log(
					"Deprecation Warning: The optional parameters of the onConnect() method are now deprecated and will be removed in the 6.0 release. Cookie parameters can be found in the main Ltijs constructor options: ... { cookies: { secure: true, sameSite: 'None' }.",
				);

			if (options.sessionTimeout || options.invalidToken)
				console.log(
					"Deprecation Warning: The optional parameters of the onConnect() method are now deprecated and will be removed in the 6.0 release. Invalid token and Session Timeout methods can now be set with the onSessionTimeout() and onInvalidToken() methods.",
				);
			...
			if (options.sessionTimeout)
				this.#sessionTimeoutCallback = options.sessionTimeout;
			if (options.invalidToken)
				this.#invalidTokenCallback = options.invalidToken;
		}
	...
	whitelist(...routes) {
		...
		this.#whitelistedRoutes = [...this.#whitelistedRoutes, ...formattedRoutes];
	...
	async registerPlatform(platform, getPlatform, ENCRYPTIONKEY, Database) {
		...
		let kid;
		const _platform = await _getPlatform(
			platform.url,
			platform.clientId,
			_ENCRYPTIONKEY,
			_Database,
		...
		if (!_platform) {
			...
			if (
				platform.authConfig.method !== "RSA_KEY" &&
				platform.authConfig.method !== "JWK_KEY" &&
				platform.authConfig.method !== "JWK_SET"
			)
				throw new Error(
					'INVALID_AUTHCONFIG_METHOD. Details: Valid methods are "RSA_KEY", "JWK_KEY", "JWK_SET".',
				);
			if (!platform.authConfig.key) throw new Error("MISSING_AUTHCONFIG_KEY");

			try {
				kid = await Auth.generatePlatformKeyPair(
					_ENCRYPTIONKEY,
					_Database,
					platform.url,
					platform.clientId,
				...
				await _Database.Replace(
					false,
					"platform",
					{ platformUrl: platform.url, clientId: platform.clientId },
					{
			...
			await _Database.Modify(
				...
				{ platformUrl: platform.url, clientId: platform.clientId },
				{
					...
					authEndpoint:
						platform.authenticationEndpoint ||
						(await _platform.platformAuthEndpoint()),
					accesstokenEndpoint:
						platform.accesstokenEndpoint ||
						(await _platform.platformAccessTokenEndpoint()),
					authorizationServer:
						platform.authorizationServer ||
						(await _platform.platformAuthorizationServer()),
					authConfig:
						platform.authConfig || (await _platform.platformAuthConfig()),
				},
			);
			return _getPlatform(
				platform.url,
	...
	async getPlatform(url, clientId, ENCRYPTIONKEY, Database) {
		...
		if (clientId) {
			...
			const platform = new Platform(
				...
				plat.clientId,
				plat.authEndpoint,
				plat.accesstokenEndpoint,
				plat.authorizationServer,
				plat.kid,
				_ENCRYPTIONKEY,
				plat.authConfig,
				_Database,
	...
	async updatePlatformById(platformId, platformInfo) {
		...
		const update = {
			...
			authenticationEndpoint:
				platformInfo.authenticationEndpoint ||
				(await platform.platformAuthEndpoint()),
			accesstokenEndpoint:
				platformInfo.accesstokenEndpoint ||
				(await platform.platformAccessTokenEndpoint()),
		};
		...
		try {
			if (alteredUrlClientIdFlag) {
				await this.Database.Modify(
					false,
					"publickey",
					{ kid: platformId },
					{ platformUrl: update.url, clientId: update.clientId },
				);
				await this.Database.Modify(
					false,
					"privatekey",
	...
	appUrl() {
		console.log(
			"Deprecation warning: The appUrl() method is now deprecated and will be removed in the 6.0 release. Use appRoute() instead.",
		);
		return this.appRoute();
	}
	...
	loginUrl() {
		console.log(
			"Deprecation warning: The loginUrl() method is now deprecated and will be removed in the 6.0 release. Use loginRoute() instead.",
		);
		return this.loginRoute();
	}
	...
	keysetUrl() {
		console.log(
			"Deprecation warning: The keysetUrl() method is now deprecated and will be removed in the 6.0 release. Use keysetRoute() instead.",
		);
		return this.keysetRoute();
	}
}
